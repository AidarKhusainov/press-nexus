...
public class PlanNewsSummary {
	...
	public Mono<SummaryPlan> execute(
		final ProcessedNews news,
		final NewsCluster cluster,
		final String lang,
		final SummarizationUseCase useCase
	) {
		return newsRepository.findReusableSummary(news.getId(), lang)
	...
	private Mono<Set<String>> loadTopRepresentativeIds() {
		...
		return Mono.zip(
				newsPipelineQuery.loadSummaryPriorityCandidates(from).collectList(),
				buildNewsClusters.execute(similarityProperties.getClusterMinScore())
			)
			.map(tuple -> {
				final Map<String, NewsSummaryPriorityCandidate> candidateById = tuple.getT1().stream()
					.collect(java.util.stream.Collectors.toMap(
						NewsSummaryPriorityCandidate::newsId,
						candidate -> candidate
					));
	...
	private ScoredRepresentative toScoredRepresentative(
		final NewsCluster cluster,
		final Map<String, NewsSummaryPriorityCandidate> candidateById
	) {
		final NewsSummaryPriorityCandidate candidate = candidateById.get(cluster.representativeId());
		if (candidate == null) {
			return null;
		}
	...
	public record SummaryPlan(
		PlanType type,
		String reason,
		CachedNewsSummary cachedSummary
	) {

		public static SummaryPlan reuseCached(final CachedNewsSummary cachedSummary, final String reason) {
			return new SummaryPlan(PlanType.REUSE_CACHED, reason, cachedSummary);
		}

		public static SummaryPlan useProvider() {
			return new SummaryPlan(PlanType.USE_PROVIDER, null, null);
		}

		public static SummaryPlan useFallback(final String reason) {
			return new SummaryPlan(PlanType.USE_FALLBACK, reason, null);
		}

		public static SummaryPlan defer(final String reason) {
