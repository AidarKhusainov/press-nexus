...
@Slf4j
@Component
public class TassPopulateContentProcessor implements NewsPopulateContentProcessor {

	private static final List<String> TASS_SELECTORS = List.of(
		"[class*=news-text] p",
		"[class*=text-block] p",
	...
	private static final Set<Media> SUPPORTED_MEDIA = Set.of(Media.TASS);

	private final WebClient webClient;

	public TassPopulateContentProcessor(final WebClientConfig webClientConfig) {
	...
	public Mono<RawNews> process(final RawNews news) {
		...
		return webClient.get()
			...
			.map(html -> HtmlContentSupport.extractArticleText(html, TASS_SELECTORS))
			.map(text -> text.isBlank() ? HtmlContentSupport.fallbackFromDescription(news) : text)
			.map(news::withRawContent)
			.onErrorResume(ex -> {
				log.warn("Сбой TASS parser id={}: {}", news.getId(), ex.getMessage());
