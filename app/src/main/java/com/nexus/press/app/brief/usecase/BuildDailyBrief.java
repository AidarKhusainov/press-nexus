...
public class BuildDailyBrief {
	...
	private final BriefCandidateQuery briefCandidateQuery;
	private final FindNearDuplicateNewsIds findNearDuplicateNewsIds;
	private final BriefToneModerationPolicy briefToneModerationPolicy;
	private final AppMetrics appMetrics;

	public Mono<DailyBrief> execute(final Request request) {
		final Request safeRequest = request == null ? new Request(null, null, null, List.of()) : request;
		final String safeLanguage = normalizeLanguage(safeRequest.language());
		final Duration safeLookback = (safeRequest.lookback() == null || safeRequest.lookback().isNegative() || safeRequest.lookback().isZero())
			? Duration.ofHours(24)
			: safeRequest.lookback();
		final int safeLimit = Math.max(1, Math.min(safeRequest.maxItems() == null ? 7 : safeRequest.maxItems(), 20));
		final Set<String> normalizedTopics = normalizeTopics(safeRequest.topics());
		final int queryLimit = Math.max(safeLimit * 8, safeLimit);

		final OffsetDateTime to = OffsetDateTime.now();
		final OffsetDateTime from = to.minus(safeLookback);

		return briefCandidateQuery.fetch(from, queryLimit, safeLanguage)
			.collectList()
			.flatMap(candidates -> findNearDuplicateNewsIds.execute(
				candidates.stream().map(BriefCandidate::id).toList(),
				BRIEF_NEAR_DUPLICATE_MIN_SCORE
			)
	...
	List<DailyBriefItem> selectItems(
		final List<BriefCandidate> candidates,
		final int maxItems,
	...
	List<DailyBriefItem> selectItems(
		...
		for (final var candidate : candidates) {
			...
			if (!matchesAnyTopic(candidate, parts, topics)) {
				continue;
			}
	...
	private boolean tryAddCandidate(
		...
		final var moderated = briefToneModerationPolicy.moderate(
			assessment.title(),
	...
	private boolean isNearDuplicate(
		final BriefCandidate candidate,
		final String title,
	...
	private boolean matchesAnyTopic(final BriefCandidate candidate, final SummaryParts parts, final Set<String> topics) {
	...
	private int importanceScore(
		...
		if (candidate.media() != null && MAJOR_MEDIA.contains(candidate.media().toUpperCase(Locale.ROOT))) {
			score += 1;
		}
		if (candidate.eventAt() != null && referenceTime != null) {
			if (candidate.eventAt().isAfter(referenceTime.minusHours(4))) {
				score += 2;
			} else if (candidate.eventAt().isAfter(referenceTime.minusHours(12))) {
				score += 1;
			}
		}
		if (hasConcreteContext(parts, language)) {
			score += 1;
		}
		if ((parts.whyImportant().length() + parts.whatNext().length()) > 180) {
			score += 1;
		}
	...
	private OffsetDateTime referenceTime(final List<BriefCandidate> candidates) {
		OffsetDateTime reference = null;
		for (final BriefCandidate candidate : candidates) {
